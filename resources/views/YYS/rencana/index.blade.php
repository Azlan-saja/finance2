                  ...
                  <table class="table border text-nowrap customize-table mb-0 align-middle table-striped">
                    <thead class="text-dark fs-4">
                      <tr class="bg-dark">
                        ...
                        <th class="border-bottom-0">
                           <a class="fw-semibold mb-0 text-white" href="{{ route('rencana.index', ['sort' => 'anggaran', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                        ...
                        <th class="border-bottom-0">
                           <a class="fw-semibold mb-0 text-white" href="{{ route('rencana.index', ['sort' => 'unit', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrows-sort" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 9l4 -4l4 4m-4 -4v14" /><path d="M21 15l-4 4l-4 -4m4 4v-14" /></svg>  
                              Ke Unit
                            </a>
                        </th>
                        <th class="border-bottom-0">                          
                            <a class="fw-semibold mb-0 text-white" href="{{ route('rencana.index', ['sort' => 'tahun', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrows-sort" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 9l4 -4l4 4m-4 -4v14" /><path d="M21 15l-4 4l-4 -4m4 4v-14" /></svg>  
                              Tahun Anggaran
                            </a>
                        </th>
                          ...
                          <div class="d-flex align-items-center gap-2">
                            <div class="dropdown dropstart">
                              ...
                              <ul class="dropdown-menu mt-0 pt-0" aria-labelledby="dropdownMenuButton">
                                ...
                                <li>
                                  <a class="dropdown-item d-flex align-items-center gap-3 text-success" href=""><i class="fs-4 ti ti-calendar-week"></i>Realisasi</a>
                                </li>
                                <li>
                                 ...
                                 <li>
                                  <form action="{{ route('rencana.closed',$data->id) }}" method="POST">
                                    @csrf  
                                    @method('PUT')                                                  
                                    <button type="submit" class="dropdown-item d-flex align-items-center gap-3 {{ $data->status == 'Open' ? 'text-bg-danger show_closed' : 'text-bg-success show_open' }}"><i class="fs-4 ti {{ $data->status == 'Open' ? 'ti-lock-off' : 'ti-lock-question' }}"></i>{{ $data->status == 'Open' ? 'Closed ?' : 'Open ?' }}</button>
                                  </form>
                                </li>
                                @if ($data->status == 'Open')
                                  <li>
                                    <a class="dropdown-item d-flex align-items-center gap-3 text-warning" href="{{ route('rencana.edit',$data->id) }}"><i class="fs-4 ti ti-edit"></i>Edit</a>
                                  </li>
                                  <li>
...
$( document ).ready(function() {
   ...
   $('.show_closed').click(function(event) {
    ...
    swal({
              ...
              text: "Jika RAB di Closed, maka data tersebut tidak bisa diubah kembali.",
              icon: "warning",
              buttons: true,
              dangerMode: true,
    })
   ...
   $('.show_open').click(function(event) {
    ...
    swal({
              ...
              text: "Jika RAB di Open, maka data tersebut bisa diubah kembali.",
              icon: "warning",
              buttons: true,
              dangerMode: true,
    })
