@extends('yys.sidebar')
@section('title')Sasaran - YYS @endsection
...
@endphp
  <div class="container mt-4">
              ...
              <div class="card mb-0">
                ...
                <!-- ISI START -->
                <a href="{{ route('sasaran.create') }}" class="btn btn-primary m-1">Tambah</a>
                <a href="{{ route('sasaran.index') }}" class="btn btn-outline-primary m-1">Segarkan</a>
                <a href="{{ route('laporan.sasaran') }}" target="_blank" class="btn btn-outline-danger m-1 position-absolute end-0 me-4">Cetak</a>                
                <hr>
                   @if ($message = Session::get('success'))
                    ...
                    @endif
                  <form action="{{ route('sasaran.search') }}" method="POST">
                     @csrf               
                ...
                <div class="table-responsive rounded-2 mb-4">
                  <table class="table border text-nowrap customize-table mb-0 align-middle table-striped">
                    <thead class="text-dark fs-4">
                      <tr class="bg-dark">
                        ...
                        <th class="border-bottom-0">                          
                            <a class="fw-semibold mb-0 text-white" href="{{ route('sasaran.index', ['sort' => 'sasaran', 'order' => $order == 'asc' ? 'desc' : 'asc']) }}">
                          ...
                          <div class="d-flex align-items-center gap-2">
                            <div class="dropdown dropstart">
                              ...
                              <ul class="dropdown-menu mt-0 pt-0" aria-labelledby="dropdownMenuButton">                               
                                <li>
                                  <span class="bg-dark  dropdown-item d-flex align-items-center gap-3 text-white text-center" >{{$data->sasaran }}</span>
                                </li>
                                <li>
                                  <a class="dropdown-item d-flex align-items-center gap-3 text-warning" href="{{ route('sasaran.edit',$data->id) }}"><i class="fs-4 ti ti-edit"></i>Edit</a>
                                </li>
                                <li>
                                  <form action="{{ route('sasaran.destroy',$data->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')                
                  ...
                  <div class="mt-3">
                     {{ $sasaran->withQueryString()->links('pagination::bootstrap-5') }}                     
                  </div>                  
                </div>
                <!-- ISI END -->
                </div>
